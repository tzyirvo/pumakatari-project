...
export class ParadaMasCercanaComponent implements OnInit {
  ...
  public smallIcon = "assets/images/small-logo.png";
  public busIcon = "assets/images/bus-marker.png";
  public route: any;
  public map: any;
  public overlays: any = [];
  public centerUpdated = false;
  public closestBus: any;
  public busPosition$: any = [];
  public busMarkerRendered = false;
  ...
  ngOnInit() {
    ...
    this.directionsRendererDirective["initialized$"].subscribe(
      directionsRenderer => {
  ...
  initializeStopsWithDest(stops) {
    ...
    stops.forEach(stop => {
      ...
      if (this.isStopinRoute(curStop.getKey(), this.route.paradas)) {
        ...
        if (!initialStopModified) {
          ...
          if (
            this.getDist(this.curPos, this.initialStop) >
            this.getDist(this.curPos, stop)
          ) {
            this.initialStop = stop;
          }
        }
    ...
    this.initStopLatLng =
      "" + this.initialStop.lat + "," + this.initialStop.lng;
    ...
    this.db.getBusesPerRoute(this.route.$key).subscribe(buses => {
      for (i = 0; i < buses.length; i += 1) {
        if (!initialBusModified) {
          ...
          this.getDist(this.initialStop, this.closestBus.posicion) >
          this.getDist(this.initialStop, buses[i].posicion)
        ) {
          this.closestBus = buses[i];
        }
      }
